...
- `quick_scan` 是严格三条的一分钟速览：第一条先说材料是什么、核心变化是什么；第二条说它为什么有用、改变了哪种做法；第三条给读者一个能带走的判断或适用边界。不要把内部审计、证据状态、质量结论或孤立数字写成速览主句，也不要复制章节标题。目录只负责章节导航。术语解释使用正文首处脚标浮层，必要原文引文才进入旁注。目录可点击空白或 Escape 收起，返回顶部控件必须真实可用。
